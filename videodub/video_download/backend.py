...
def build_download_command(
    ...
    *,
    ...
    command = [
        ...
        "--write-info-json",
        # Applying the video template to playlist metadata creates an invalid
...
def _keep_one_source_subtitle(job: VideoJob, runner: ProcessRunner) -> None:
    ...
    for candidate in job.video_path.parent.glob(job.video_path.stem + "*.srt"):
        if candidate != selected and not candidate.name.endswith(generated_suffixes):
...
def download(config: AppConfig, runner: ProcessRunner, url: str) -> list[VideoJob]:
    ...
    try:
        ...
        subtitle_failure = (
            ...
            and any(
                marker in detail
                for marker in ("unable", "failed", "error", "too many requests")
            )
        )
        ...
        lines = runner.run(build_download_command(config, url, include_subtitles=False))
    ids = list(
        dict.fromkeys(
